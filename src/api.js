...
const getToken = async (code) => {
  ...
  const { access_token } = await fetch(
    "https://5bsexv41pb.execute-api.eu-central-1.amazonaws.com/dev/api/token/" + encodeCode
  )
    .then((res) => {
      return res.json();
    })
...
export const getEvents = async () => {
  ...
  if(token) {
    ...
    removeQuery();
    // 
    const url = "https://5bsexv41pb.execute-api.eu-central-1.amazonaws.com/dev/api/get-events/" + token;
